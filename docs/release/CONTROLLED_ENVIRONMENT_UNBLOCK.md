...
It prints only setting names and validation reasons. It never returns secret values. A zero exit code means configuration is structurally ready to start controlled execution; it does not mean a suite passed. The preflight now matches the API/Worker production startup boundary for PostgreSQL TLS, minimum secret lengths, the 32-byte address key, Worker tenant UUID and exact non-blanket trusted-proxy CIDRs, and its protected workflow rejects a configured release SHA that differs from the requested candidate. `0.0.0.0/0`, `::/0` and unspecified proxy addresses are invalid because they would allow client-address spoofing. Once all performance settings exist, the same production-shaped performance validator also checks the candidate manifest, three GHCR digests, deployed-image equality, safe API paths/request bodies and a JSON report path before any load is sent.

Run `pnpm controlled:inventory` before provisioning to generate the canonical names-only Stage 47–50 inventory directly from the protected workflow. To audit a server/account export without exposing values, create a JSON file containing only `secrets`, `variables` and `externalFiles` name arrays, then run `pnpm controlled:inventory -- --names=<absolute-or-relative-json-path>`. With a read-capable GitHub token in the process environment, `pnpm controlled:inventory -- --github=<owner/repository>` audits the protected environment directly. GitHub never exposes secret values; variable values present in its list response are discarded before the report is built. The report contains names only and exits non-zero for missing or unexpected names, secret/variable misclassification, cross-class ambiguity, malformed responses or a list truncated beyond the enforced 100-name completeness boundary.

The generated `counts` block makes the provisioning denominator explicit: 58 per-stage references collapse to 57 unique requirements because `RELEASE_COMMIT` is shared by Stages 49 and 50. Of those unique requirements, 56 are GitHub Environment names and one is the externally supplied `CONTROLLED_RESULTS_FILE`. These counts are schema-tested so an operator does not confuse workflow settings with the external evidence file or count a shared name twice.
