...
| 47–50  | Still controlled pending. The reviewed preflight is on `main`; `controlled-preproduction` requires review, prevents self-review and is restricted to the `main` workflow ref. Application-owned keys and the candidate binding are provisioned without exposing values. The 2026-08-20 read-only name audit gives coverage of 2/15, 3/9, 7/32 and 1/2 for Stages 47–50. Stage 49 now also requires the Worker tenant UUID plus candidate and actually deployed image-digest snapshots. A protected final workflow validates a candidate-bound draft evidence archive with trusted `main` verifier code and emits an attestation, but it is not yet reviewed/executed. Real provider/deployment values, a second reviewer and controlled results remain absent, so no external PASS is claimed.                   |
...
The final local repository gate is green after production-runtime, identity risk/audit, legacy-inventory, result-provenance, exact evidence-package staging, candidate-image publication, protected launch verification, development-mock isolation and cloud-workspace hardening: formatting, lint, every static contract, 98 test files/493 tests, eight type/contract checks and all six production builds passed. `launch:gate` remains fail-closed without the eleven real, independently reviewed suite bundles. The development mock gateway and PostgreSQL-backed Dev Container unblock provider-dependent application work but are explicitly separated from production runtime and controlled preflight. The controlled plan now also requires signed financial-policy approvals and the approved live legal-document publication matrix; neither may be replaced by provider sandbox or local privacy tests. Result schema v3 binds different opaque executor/reviewer subjects, ordered review time, artifact hashes and capture-receipt hashes before the non-overwriting assembler can package evidence. Candidate checks are bound to the exact SHA, trusted-identical CI workflow, GitHub Actions app and same-repository workflow run rather than accepted by display name alone, while every external workflow Action, Dockerfile frontend and production/CI/development base image is fixed to a centrally approved immutable commit or manifest SHA. Stage 49 preflight validates the complete performance topology and locks the current-policy publisher Run, unique Artifact ID and GitHub Artifact SHA-256 before safely reading its single manifest, so hand-authored or replaced image manifests cannot satisfy readiness. The protected final verifier similarly locks the unique Release/Asset IDs and attests the downloaded archive hash, preventing a tag/name replacement from changing the evidence under review. The protected publisher and final verifier are locally verified but cannot publish/attest until reviewed onto trusted `main` and an independent environment reviewer approves execution.
