...
Place `results.json` at the evidence root and each declared artifact under the exact suite directory/name from `controlled-acceptance-plan.json`. The result manifest uses version 1 and contains:
...
- suite code, exact environment gate, `PASS`, execution timestamps, the planned executor role and a different independent reviewer role;
- every declared evidence path and the SHA-256 of its actual file.

Extra suites, missing suites, missing/extra artifacts, path traversal, hash mismatch, self-review, future timestamps, a changed plan or a different candidate commit all fail closed. Evidence paths are relative to `results.json`; do not use absolute paths or links.
