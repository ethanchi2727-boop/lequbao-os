...
The trusted `.github/workflows/publish-candidate-images.yml` workflow publishes the exact checked candidate only from the protected `main` workflow ref and `controlled-preproduction` environment. It requires successful `code-quality`, `postgres-contract` and `container-build` check runs before receiving package write permission. API, Worker and Web are published to GHCR with a unique candidate/run tag, BuildKit provenance and SBOM attestations; `candidate-image-digests.json` records the three immutable digest references. Deploy only those digest references, never the convenience tag. A new run creates a new tag and manifest rather than overwriting prior release evidence.
...
- Stage 49 preflight also requires the successful candidate-image publisher run ID. It verifies the run came from the trusted `main` publisher workflow, downloads the uniquely named digest artifact and compares it with the configured performance manifest. Do not transcribe or recreate digest JSON manually.
- After all eleven controlled suites are independently reviewed and the local launch precheck passes, use `pnpm controlled:stage-release` to create an exact upload-only bundle, attach its archive to a candidate-bound draft GitHub Release and dispatch `.github/workflows/verify-controlled-release.yml` from trusted `main`. The protected job rejects unsafe, missing or undeclared package entries, executes only trusted verifier code, records the exact results/plan hashes and produces an OIDC-backed attestation. A self-reported reviewer field or a local green command is not a production approval.
