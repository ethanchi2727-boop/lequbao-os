import { describe, expect, it } from 'vitest';
import { dirname, join } from 'node:path';
import { fileURLToPath } from 'node:url';
import { verifyLocalReleaseGateSequence } from './verify-local-release-gate-sequence.mjs';

describe('local release gate sequence', () => {
  it('locks fifteen ordered local/static gates and excludes controlled evidence execution', async () => {
    const root = join(dirname(fileURLToPath(import.meta.url)), '..');
    const report = await verifyLocalReleaseGateSequence(root);
    expect(report.sequence).toHaveLength(15);
    expect(report.checks).toHaveLength(16);
    expect(report.checks.every((check) => check.passed)).toBe(true);
    expect(report.checks.at(-1).name).toBe('controlled-execution-gates-excluded');
  });
});
