...
function invalidValue(environment, name) {
  ...
  if (/replace-with|example-secret|changeme/iu.test(value))
    return "placeholder";
  if (secretPattern.test(name) && Buffer.byteLength(value, "utf8") < 16)
    return "too-short";
  ...
  if (name === "RELEASE_COMMIT" && !/^[a-f0-9]{40}$/u.test(value))
    return "invalid-commit";
  if (name === "CONTROLLED_RESULTS_FILE" && !path.isAbsolute(value))
    return "not-absolute";
  return undefined;
}

export function inspectControlledEnvironment(
  environment,
  stages = [47, 48, 49, 50],
) {
  const names = [
