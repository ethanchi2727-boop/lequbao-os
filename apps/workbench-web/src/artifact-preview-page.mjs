...
const enumZhLabels = Object.freeze({
  ASSIGNED: '已派单',
  COMPLETED: '已完成',
  CONFIRMED: '已确认',
  CONFLICT: '冲突',
  ...
  URGENT: '紧急',
  VOUCHER_USE: '券核销',
  WAITING_ANSWERS: '等待补充回答',
  WAITING_APPROVAL: '等待审批',
  WAITING_CONFIRMATION: '等待人工确认',
  WAITING_CUSTOMER: '等待客户回复',
  WEB: '网页',
  WECHAT_MINI_PROGRAM: '微信小程序',
  BUSINESS_RECORD: '经营记录',
  HUMAN_CONFIRMATION: '人工确认记录',
  TOOL_RESULT: '工具执行结果',
  COMPLEX: '复杂任务',
});
const enumZh = (value) => {
  ...
  if (!key) return '—';
  return enumZhLabels[key] ?? '待确认';
};
const failureCodeZhLabels = Object.freeze({
  PROVIDER_TIMEOUT: '供应商超时',
  UPSTREAM_TIMEOUT: '上游服务超时',
  TOOL_TIMEOUT: '工具执行超时',
  UNKNOWN_EXTERNAL_RESULT: '外部结果未知',
});
const scanSecurityZhLabels = Object.freeze({
  SAFE: '扫描通过',
  REJECTED: '已拦截',
  PENDING: '排队待扫',
  QUEUED: '排队待扫',
...
const scanSecurityZh = (value) =>
  scanSecurityZhLabels[String(value ?? '').toUpperCase()] ?? '待扫描';
const processingStatusZhLabels = Object.freeze({
  SUCCEEDED: '处理完成',
  FAILED: '处理失败',
  QUEUED: '排队中',
  PENDING: '排队中',
...
const processingStatusZh = (value) =>
  processingStatusZhLabels[String(value ?? '').toUpperCase()] ?? '待处理';
const errorCodeZhLabels = Object.freeze({
  MALWARE_DETECTED: '检测到恶意文件',
});
const errorCodeZh = (value) => errorCodeZhLabels[String(value ?? '').toUpperCase()] ?? '扫描未通过';
const assetTypeZhLabels = Object.freeze({
  DOCUMENT: '文档',
  IMAGE: '图片',
  AUDIO: '语音',
  VIDEO: '视频',
...
const assetTypeZh = (value) => assetTypeZhLabels[String(value ?? '').toUpperCase()] ?? '材料';
const authLevelZhLabels = Object.freeze({
  MFA: '二次核验',
  PASSWORD: '密码',
});
const authLevelZh = (value) => authLevelZhLabels[String(value ?? '').toUpperCase()] ?? '已认证';
const roleZhLabels = Object.freeze({
...
const roleZh = (value) => roleZhLabels[String(value ?? '').toUpperCase()] ?? '员工';
const failureCodeZh = (value) => {
  const key = String(value ?? '').toUpperCase();
  if (!key) return '执行异常';
  return failureCodeZhLabels[key] ?? '执行异常';
};

const impactTargetLabels = Object.freeze({
  MINI_PROGRAM: '小程序',
  GEO: 'GEO',
  AI_CUSTOMER_SERVICE: 'AI 客服',
});
const impactTargetLabel = (target) => impactTargetLabels[target] ?? '业务服务';
...
const intakeSessionStatusLabel = (status) =>
  intakeSessionStatusLabels[String(status ?? '').toUpperCase()] ?? '等待服务端状态';
...
export function renderConversationThread({ contract, demoMode, livePageState, view, escapeHtml }) {
  ...
  return `<section class="artifact-page" data-page-id="${contract.id}" data-experience="artifact-preview">
    <header class="artifact-heading"><div><small>${demoMode ? '演示成果 · 非真实业务数据' : '权威任务成果'}</small><h1>本周经营复盘</h1><p>先核对版本、摘要、来源和交付边界，再进入任何发布动作。</p></div><div class="artifact-heading-actions"><span class="artifact-status">${escapeHtml(enumZh(status))}</span><a href="${taskHref}">返回任务</a></div></header>
    <div class="artifact-layout">
      ...
      <aside class="artifact-inspector">
        <section><header><div><small>版本摘要</small><strong>计划版本 ${escapeHtml(planVersion)}</strong></div><span>${escapeHtml(enumZh(status))}</span></header><dl><div><dt>任务编号</dt><dd>${escapeHtml(taskId)}</dd></div><div><dt>完成时间</dt><dd>${escapeHtml(dateText(completedAt))}</dd></div><div><dt>执行结果</dt><dd>${succeededSteps}/${steps.length} 步成功</dd></div></dl></section>
...
function renderProvenancePath(path, candidates, assets, escapeHtml) {
  return `<article class="provenance-path"><header><code>${escapeHtml(path)}</code><span>${candidates.length} 个候选</span></header><div>${candidates
    .map((field, index) => {
      ...
      return `<section class="provenance-candidate status-${String(field.decisionStatus).toLowerCase()}"><b>${index + 1}</b><div><header><strong>${escapeHtml(text(field.candidateValue, '空候选'))}</strong><span>${escapeHtml(decisionStatusLabel(field.decisionStatus))}</span></header><p>置信度 ${field.confidence == null ? '未返回' : `${Math.round(Number(field.confidence) * 100)}%`} · 候选 ${escapeHtml(field.id ?? '未知')}</p><footer><span>${escapeHtml(asset?.originalFilename ?? '未知材料')}</span><code>${escapeHtml(String(asset?.sha256 ?? '无哈希').slice(0, 12))}…</code><em>${escapeHtml(scanSecurityZh(asset?.securityStatus))} / ${escapeHtml(processingStatusZh(asset?.processingStatus))}</em></footer></div></section>`;
    })
    .join('')}</div></article>`;
}
...
function renderConfirmationField(field, escapeHtml) {
  ...
  return `<article class="confirmation-field status-${String(field.decisionStatus).toLowerCase()}"><header><code>${escapeHtml(path)}</code><span>${risk}</span></header><div><section><small>当前正式值</small><strong>尚未写入正式档案</strong><p>本接口不返回历史正式值，不在客户端推测。</p></section><b>→</b><section><small>待确认候选 · ${confidence}</small><strong>${escapeHtml(text(field.candidateValue, '空候选'))}</strong><p>${escapeHtml(decisionStatusLabel(field.decisionStatus))} · 编号 ${escapeHtml(field.id ?? '未知')}</p></section></div></article>`;
}
    ...
    confidence === null
      ...
      : confidence >= 90
        ...
        : confidence >= 70
          ? '需核对'
          : '低信心';
  return `<article class="recognition-field status-${String(field.decisionStatus).toLowerCase()}"><header><div><code>${escapeHtml(field.fieldPath ?? '未知字段')}</code><span>${escapeHtml(decisionStatusLabel(field.decisionStatus))}</span></div><b>${confidence === null ? '—' : `${confidence}%`}</b></header><h3>${escapeHtml(text(field.candidateValue, '空候选'))}</h3><footer><span>${tier}</span><small>来源：${escapeHtml(asset?.originalFilename ?? String(field.sourceAssetId ?? '未知材料'))}</small></footer></article>`;
}
...
function renderIdentitySwitcher({ contract, demoMode, livePageState, view, escapeHtml }) {
  ...
  return `<section class="identity-page" data-page-id="${contract.id}" data-experience="identity-switcher">
    <header class="identity-heading"><div><small>${demoMode ? '演示身份 · 非真实会话' : '服务端已确认'}</small><h1>身份与工作空间</h1><p>切换前核对当前租户、门店、角色和会话边界。</p></div><span>${escapeHtml(authLevelZh(context.authLevel))}</span></header>
    <div class="identity-grid"><main><section class="identity-current"><div class="identity-avatar" aria-hidden="true">${escapeHtml(String(context.userId).slice(0, 2).toUpperCase())}</div><div><small>当前认证主体</small><h2>员工 ${escapeHtml(String(context.userId).slice(0, 8))}…</h2><p>会话 ${escapeHtml(String(context.sessionId).slice(0, 12))}… · ${escapeHtml(authLevelZh(context.authLevel))}认证</p></div><b>在线</b></section>
      <section class="identity-scope"><header><div><small>当前工作空间</small><h2>${escapeHtml(context.tenantId)}</h2></div><span>租户级边界</span></header><div class="identity-facts"><article><small>角色能力</small><strong>${roles.length} 项</strong><div>${roles.map((role) => `<span>${escapeHtml(roleZh(role))}</span>`).join('')}</div></article><article><small>门店范围</small><strong>${stores.length || '租户级'}</strong><div>${stores.length ? stores.map((store) => `<code>${escapeHtml(String(store).slice(0, 8))}…</code>`).join('') : '<span>不限定单一门店</span>'}</div></article></div></section>
...
function renderConversationRow(item, demoMode, escapeHtml) {
  ...
  const statusCopy =
    {
      ...
      CLOSED: '已完成',
    }[item.status] ?? enumZh(item.status);
  ...
  return `<article data-conversation-row data-search="${escapeHtml(search)}"><div class="session-avatar">客</div><div class="session-row-main"><header><strong>客户 · ${escapeHtml(customer.slice(-6))}</strong><time>${escapeHtml(dateText(item.updatedAt))}</time></header><p>${escapeHtml(enumZh(ticket?.reasonCode ?? 'GENERAL_SUPPORT'))} · 内容需进入会话后按权限读取</p><footer><span>${escapeHtml(enumZh(item.channel))}</span><span>${escapeHtml(enumZh(item.contextType ?? 'NONE'))}</span>${item.riskLevel !== 'NORMAL' ? `<span class="risk">${escapeHtml(enumZh(item.riskLevel))}</span>` : ''}</footer></div><div class="session-row-status"><b>${escapeHtml(statusCopy)}</b>${ticket ? `<small class="priority-${String(ticket.priority).toLowerCase()}">${escapeHtml(enumZh(ticket.priority))} · ${escapeHtml(enumZh(ticket.status))}</small><time>${escapeHtml(ticket.dueAt ? `截止 ${dateText(ticket.dueAt)}` : '无截止时间')}</time>` : '<small>无转人工工单</small>'}<a href="${href}" aria-label="打开客户 ${escapeHtml(customer.slice(-6))} 的会话">打开会话</a></div></article>`;
}
...
function renderProvenance({ contract, demoMode, livePageState, view, escapeHtml }) {
  ...
  return `<section class="provenance-page" data-page-id="${contract.id}" data-experience="provenance-timeline">
    <header class="provenance-heading"><div><small>${demoMode ? '演示轨迹 · 非真实业务数据' : '权威任务轨迹'}</small><h1>来源与工具轨迹</h1><p>按执行顺序核对读取范围、工具结果、权限边界和人工确认。</p></div><span>${escapeHtml(enumZh(read(task, 'status')))}</span></header>
...
function renderStep(step, escapeHtml) {
  ...
  return `<article class="step-record"><b>${escapeHtml(number)}</b><div><header><strong>${escapeHtml(action)}</strong><span>${escapeHtml(enumZh(status))}</span></header><p>${escapeHtml(output)}</p><footer><code>${escapeHtml(tool ?? '无工具调用')}</code><small>风险 ${escapeHtml(enumZh(risk))} · 尝试 ${escapeHtml(attempts)} 次${failure ? ` · 失败原因：${escapeHtml(failureCodeZh(failure))}` : ''}</small></footer></div></article>`;
}

function renderTimelineEvidence(item, escapeHtml) {
  return `<article><time>${escapeHtml(dateText(read(item, 'created_at', 'createdAt')))}</time><i></i><div><small>${escapeHtml(enumZh(read(item, 'evidence_type', 'evidenceType') ?? ''))}</small><h3>${escapeHtml(item.label ?? '未命名证据')}</h3><p>${escapeHtml(text(read(item, 'summary_redacted', 'summaryRedacted'), '无可展示的脱敏摘要'))}</p><code>${escapeHtml(String(read(item, 'reference_hash', 'referenceHash') ?? '').slice(0, 24))}</code></div></article>`;
}
...
function renderArtifact(item, escapeHtml) {
  ...
  return `<article><b>${escapeHtml(
    String(item.name ?? '未命名成果')
      ...
      .at(-1)
      ?.toUpperCase(),
  )}</b><div><strong>${escapeHtml(item.name ?? '未命名成果')}</strong><small>${escapeHtml(enumZh(item.status))} · ${size ? `${Math.ceil(size / 1024)} KB` : '大小未知'}</small><code>SHA ${escapeHtml(hash.slice(0, 12))}</code></div></article>`;
}
