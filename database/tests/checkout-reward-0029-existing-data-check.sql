...
BEGIN
  ...
  ) THEN RAISE EXCEPTION '0029 changed existing redemption facts'; END IF;
  IF NOT EXISTS (
  ...
  ) THEN RAISE EXCEPTION '0029 changed existing reward facts'; END IF;
  IF NOT EXISTS (
    SELECT 1 FROM schema_migrations WHERE version='0029_checkout_reward_redemption_scope'
  ) THEN RAISE EXCEPTION '0029 marker missing'; END IF;
END
$$;

\echo '0029 preserved committed 0028 redemption and reward facts'
