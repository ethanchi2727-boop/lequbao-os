...
ON revenue_payout_attempts(tenant_id, allocation_id) WHERE status = 'SUCCEEDED';
...
DECLARE original_allocation uuid; original_type text; original_amount bigint;
BEGIN
  ...
  IF NEW.entry_type IN ('PAYMENT','REVERSAL') THEN
    IF NEW.original_entry_id IS NULL THEN RAISE EXCEPTION '% entry requires an original entry', NEW.entry_type; END IF;
    SELECT allocation_id, entry_type, amount_cents INTO original_allocation, original_type, original_amount
      FROM revenue_distribution_entries WHERE tenant_id = NEW.tenant_id AND id = NEW.original_entry_id;
...
END;
$$;
CREATE TRIGGER revenue_distribution_entry_linkage_check BEFORE INSERT ON revenue_distribution_entries
...
BEGIN
  ...
  IF NOT ((OLD.status = 'PENDING' AND NEW.status IN ('APPROVED','REJECTED','EXPIRED'))
       OR (OLD.status = 'APPROVED' AND NEW.status IN ('CONSUMED','EXPIRED'))) THEN
...
RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE allocation_statement uuid; allocation_amount bigint; approval_statement uuid;
        approval_action text; approval_status text; approval_expiry timestamptz;
BEGIN
  SELECT statement_id, allocated_cents INTO allocation_statement, allocation_amount
    FROM revenue_distribution_allocations WHERE tenant_id = NEW.tenant_id AND id = NEW.allocation_id;
  SELECT statement_id, action_type, status, expires_at
    INTO approval_statement, approval_action, approval_status, approval_expiry
    FROM revenue_distribution_action_approvals WHERE tenant_id = NEW.tenant_id AND id = NEW.approval_id;
...
END;
$$;
CREATE TRIGGER revenue_payout_evidence_check BEFORE INSERT ON revenue_payout_attempts
...
CREATE TABLE merchant_intake_sessions (
  ...
  impact_targets text[] NOT NULL DEFAULT '{}'
    CHECK (impact_targets <@ ARRAY['MINI_PROGRAM','GEO','AI_SERVICE','PRODUCT','GROUP_BUY']::text[]),
  version integer NOT NULL DEFAULT 1,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (tenant_id, id),
...
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF OLD.tenant_id <> NEW.tenant_id OR OLD.channel <> NEW.channel OR OLD.created_by <> NEW.created_by
     OR OLD.created_at <> NEW.created_at OR OLD.delivery_project_id IS DISTINCT FROM NEW.delivery_project_id THEN
  ...
  ) THEN RAISE EXCEPTION 'invalid merchant intake transition % to %', OLD.status, NEW.status; END IF;
...
END;
$$;
CREATE TRIGGER merchant_intake_session_transition_check BEFORE UPDATE ON merchant_intake_sessions
...
BEGIN
  IF TG_OP = 'UPDATE' THEN
    IF OLD.tenant_id <> NEW.tenant_id OR OLD.session_id <> NEW.session_id OR OLD.source_channel <> NEW.source_channel
       OR OLD.source_message_id IS DISTINCT FROM NEW.source_message_id OR OLD.asset_type <> NEW.asset_type
       OR OLD.object_key IS DISTINCT FROM NEW.object_key OR OLD.sha256 <> NEW.sha256
       OR OLD.created_by <> NEW.created_by OR OLD.created_at <> NEW.created_at THEN
    ...
    ) THEN RAISE EXCEPTION 'invalid asset processing transition % to %', OLD.processing_status, NEW.processing_status; END IF;
  END IF;
...
END;
$$;
CREATE TRIGGER merchant_intake_asset_processing_check BEFORE INSERT OR UPDATE ON merchant_intake_assets
...
BEGIN
  IF TG_OP = 'UPDATE' THEN
    IF OLD.tenant_id <> NEW.tenant_id OR OLD.session_id <> NEW.session_id OR OLD.asset_id <> NEW.asset_id
       OR OLD.field_path <> NEW.field_path OR OLD.candidate_value <> NEW.candidate_value
    ...
    ) THEN RAISE EXCEPTION 'invalid candidate decision transition % to %', OLD.decision_status, NEW.decision_status; END IF;
  END IF;
  SELECT session_id, security_status, processing_status INTO asset_session, asset_security, asset_processing
    FROM merchant_intake_assets WHERE tenant_id = NEW.tenant_id AND id = NEW.asset_id;
...
END;
$$;
CREATE TRIGGER merchant_intake_candidate_trace_check BEFORE INSERT OR UPDATE ON merchant_intake_field_candidates
...
BEGIN
  ...
  IF session_status <> 'CONFIRMED' THEN RAISE EXCEPTION 'merchant intake session must be confirmed before commit'; END IF;
  IF NOT EXISTS (SELECT 1 FROM merchant_intake_confirmations WHERE tenant_id = NEW.tenant_id
                 AND session_id = NEW.session_id AND confirmation_type = 'LEGAL_SUBJECT') THEN
    RAISE EXCEPTION 'legal subject confirmation is required before commit';
  END IF;
  IF EXISTS (SELECT 1 FROM merchant_intake_field_candidates WHERE tenant_id = NEW.tenant_id
             AND session_id = NEW.session_id AND decision_status IN ('PROPOSED','CONFLICT')) THEN
    RAISE EXCEPTION 'unresolved merchant intake candidates block commit';
  END IF;
  IF EXISTS (
    SELECT 1 FROM merchant_intake_field_candidates candidate
     WHERE candidate.tenant_id = NEW.tenant_id AND candidate.session_id = NEW.session_id
       AND candidate.decision_status <> 'REJECTED'
       AND CASE WHEN candidate.field_path LIKE 'payment.%' THEN 'PAYMENT'
                ...
                WHEN candidate.field_path LIKE 'publish.%' THEN 'PUBLISH_IMPACT' END IS NOT NULL
       AND NOT EXISTS (
         SELECT 1 FROM merchant_intake_confirmations confirmation
          WHERE confirmation.tenant_id = candidate.tenant_id AND confirmation.session_id = candidate.session_id
            AND confirmation.confirmation_type = CASE WHEN candidate.field_path LIKE 'payment.%' THEN 'PAYMENT'
              WHEN candidate.field_path LIKE 'product.%price%' THEN 'PRICE'
              WHEN candidate.field_path LIKE 'refund.%' THEN 'REFUND_RULE'
              WHEN candidate.field_path LIKE 'merchant.public_contact.%' THEN 'PUBLIC_CONTACT'
              WHEN candidate.field_path LIKE 'publish.%' THEN 'PUBLISH_IMPACT' END
       )
  ) THEN RAISE EXCEPTION 'high-risk merchant intake fields require separate confirmations'; END IF;
  RETURN NEW;
END;
$$;
CREATE TRIGGER merchant_intake_commit_ready_check BEFORE INSERT ON merchant_intake_commits
...
\ir migrations/0030_checkout_reward_customer_scope.sql
